...
public class Path {
    ...
    public static List<BlockPos> calculateBresenhamVoxels(BlockPos startPos, BlockPos endPos) {
        ...
        BridgingAdjacency adjacencySupported = BridgingConfig.BRIDGING_ADJACENCY.get();
        float extentMult = BridgingConfig.BRIDGING_SNAP_STRENGTH.get().floatValue();
        ...
        if (dx >= dy && dx >= dz) {
            ...
            while (Math.abs(workingVec.x() - targetVec.x()) > NEAR_ZERO) {
                ...
                if(adjacencySupported != BridgingAdjacency.NONE) {
