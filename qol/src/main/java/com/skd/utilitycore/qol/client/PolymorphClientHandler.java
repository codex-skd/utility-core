package com.skd.utilitycore.qol.client;
...
public class PolymorphClientHandler {
    ...
    public static void onMousePressed(ScreenEvent.MouseButtonPressed.Pre event) {
        ...
        if (button == 0) {
            ...
            if (cachedRecipes.size() > 1) {
                UtilityCoreQoL.LOGGER.info("[RecipeSelector] Click: mouse=({}, {}) selector=({}, {}) cols={} cached={} inX={} row={} col={} index={}",
            ...
            if (cachedRecipes.size() > 1 && inX && row >= 0) {
                if (index >= 0 && index < cachedRecipes.size()) {
                    ...
                    if (row < rows && mouseY >= selectorY && mouseY < selectorY + rows * 18) {
                        ...
                        UtilityCoreQoL.LOGGER.info("[RecipeSelector] SELECT index={} (old selectedIndex={}) cached={} -> SelectRecipePacket sent",
                                index, oldIndex, cachedRecipes.size());
                        event.setCanceled(true);
                    } else {
                        UtilityCoreQoL.LOGGER.info("[RecipeSelector] Click outside selector grid: row={} rows={} mouseY={} selY={}", row, rows, mouseY, selectorY);
                    }
                } else {
                    UtilityCoreQoL.LOGGER.info("[RecipeSelector] Click index out of range: index={} cached={}", index, cachedRecipes.size());
                }
            } else if (cachedRecipes.size() > 1) {
                UtilityCoreQoL.LOGGER.info("[RecipeSelector] Click missed selector bounds: inX={} row={} (mouse={}, {})",
    ...
    public static void updateRecipeCache(CraftingContainer container, Minecraft mc) {
        ...
        if (inputsChanged(lastInputs, inputs)) {
            ...
            try {
                ...
                UtilityCoreQoL.LOGGER.error("Error finding recipes: {}", e.getMessage());
                cachedRecipes.clear();
                lastInputs.clear();
                return;
            }
            UtilityCoreQoL.LOGGER.info("[RecipeSelector] updateRecipeCache: inputs changed, found {} recipes (selectedIndex={})",
    ...
    private static void setRecipeOutputs(List<RecipeHolder<CraftingRecipe>> recipes, CraftingInput input) {
        int max = QoLConfig.MAX_RECIPES_DISPLAYED.get();
        cachedRecipes.clear();
    ...
    public static ItemStack renderRecipeSelector(GuiGraphicsExtractor extractor, Minecraft mc, int mouseX, int mouseY,
        ...
        if (recipes.size() <= 1) {
            if (selectorWasVisible) {
                UtilityCoreQoL.LOGGER.info("[RecipeSelector] Selector hidden (recipes={})", recipes.size());
        ...
        if (!wasVisible || selectedIndex != lastRenderSelected) {
            UtilityCoreQoL.LOGGER.info("[RecipeSelector] Render: pos=({}, {}) cols={} rows={} recipes={} selected={} {}",
