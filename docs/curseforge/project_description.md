...
<li>Read from <code>utility_core/spawn_schem/schematic_spawn.schem</code> in the server's game directory — the mod no longer bundles a default schematic, so you must place your own <code>.schem</code> file there before creating a new world</li>
...
<tr><td><code>spawnSchematic.surfaceOffset</code></td><td>70</td><td>Vanilla / All</td><td>Extra blocks above the ground where the bottom of the schematic is placed (only in <code>SURFACE</code> mode). 0 = flush, positive = raised, negative = buried.</td></tr>
...
<tr><td><code>spawnSchematic.protectionEnabled</code></td><td>true</td><td>Vanilla / All</td><td>Build protection of the spawn area (no breaking/placing, no explosions). Toggle at runtime with <code>/utilitycore spawnprotection on|off</code>.</td></tr>
...
<li>For Spawn Schematic: set <code>enableSpawnSchematic=true</code> and place your own <code>.schem</code> file at <code>utility_core/spawn_schem/schematic_spawn.schem</code> in the game directory (the mod no longer bundles a default). Then create a <strong>new world</strong>. Use <code>/utilitycore spawnprotection off</code> to build inside the protected area and <code>/utilitycore spawnprotection on</code> to re-enable protection. <blockquote><strong>To apply to an existing world, you must delete the world save folder first</strong> — this is intentional.</blockquote></li>
